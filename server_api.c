...
// log_printf: prints a timestamp and a message; note that the timestamp is in
// the local time zone.
...
// is_service_registered: returns 1 if the given service id is registered, 0
// otherwise.
int is_service_registered(int svcid) {
  if (svcid < 0 || svcid >= MAX_SERVICES) {
    return 0; // invalid service id
  }
  if (services[svcid] == 1) {
...
// if the array is full, it evicts the oldest record (at index 0) by shifting
// left.
...
// handle_unregistered_service: handles requests for services that are not
// registered.
...
// handle_cancel_request: handles cancel requests (svcid == -1) by removing the
// request from queues.
...
void *udp_listener_thread(void *arg) {
  ...
  while (1) {
    ...
    log_printf("[udp listener] received reqid: %d, svcid: %d, reqlen: %d, "
               "reqbuf: %s\n",
    ...
    if (req.reqid < 0) {
      ...
      while (current != open_tail) {
        ...
        if (tmp.reqid == ((-1) * (req.reqid))) {
          // acknowledge the ack
          log_printf(
              "[udp listener] ack was received for heartbeat for reqid %d\n",
              req.reqid);
    ...
    // update processed reqs (this function encapsulates the insertion and fifo
    // replacement logic)
    if (req.svcid != -1) {
...
int register_service(int svcid) {
  if (svcid < 0 || svcid >= MAX_SERVICES) {
    log_printf("error: invalid svcid %d (must be 0-%d)\n", svcid,
               MAX_SERVICES - 1);
    return -1;
  }
  services[svcid] = 1;
...
// copies the request payload into buf (up to *len bytes), updates *len, and
// returns the reqid.
...
// sendReply: sends a reply for the given reqid by enqueuing the response for
// the UDP sender thread.
